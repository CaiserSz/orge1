...
- **Meter Durumu**
  - Fiziksel enerji ölçer (meter) entegrasyonu henüz tamamlanmadı; bu nedenle sahadaki testlerde kWh/enerji değerleri harici metre ile doğrulanmadı.
  - Mevcut durumda sistem, enerji/meter değerleri olmadan da tam fonksiyonel şarj senaryosunu (yetkilendirme, state yönetimi, session/metrik kayıtları) başarıyla çalıştırabiliyor.
  - Meter okuma ve entegrasyon çalışmaları `todo/master_next.md` altında Faz 4'te planlı:
    - `meter/read_meter.py` geliştirme ve gerçek cihazla entegrasyon
    - `meter/read_meter.py` modüllere bölünerek standartlara uygun hale getirilmesi
