...
**Son Değiştirilme Tarihi:** 2025-12-08 17:17:00  
**Version:** 1.3.6  
...
**TCP Tunnel:**
- **Public URL:** `tcp://9.tcp.eu.ngrok.io:29229` (Reserved Address)
...
**SSH Tunnel:**
- **Public URL:** `tcp://2.tcp.eu.ngrok.io:11286` (Dinamik adres, servis yeniden başlatıldığında değişebilir)
- **Local Address:** `localhost:22` (SSH servisi)
- **Durum:** ✅ Aktif ve dışarıdan erişilebilir
- **Kullanım:** Dışarıdan SSH erişimi için kullanılabilir
...
- **Kullanım:** Web servisleri, API'ler vb.

**SSH Erişimi:**
- **Adres:** `2.tcp.eu.ngrok.io:11286` (dinamik, servis yeniden başlatıldığında değişebilir)
- **Hedef:** RPi SSH servisi (port 22)
- **Kullanım:** Dışarıdan SSH bağlantısı
- **SSH Komutu:**
  ```bash
  ssh -p 11286 kullanici_adi@2.tcp.eu.ngrok.io
  ```
- **Not:** SSH tunnel adresi dinamik olduğu için, güncel adresi öğrenmek için:
  ```bash
  curl -s http://localhost:4040/api/tunnels | python3 -c "import sys, json; data=json.load(sys.stdin); tunnels=[t for t in data.get('tunnels', []) if t.get('name')=='ssh']; print(tunnels[0]['public_url'] if tunnels else 'SSH tunnel bulunamadı')"
  ```

**TCP Erişimi (Genel):**
- **Adres:** `9.tcp.eu.ngrok.io:29229` (reserved address, sabit)
- **Hedef:** RPi'deki port 29229
- **Kullanım:** Özel TCP servisleri
