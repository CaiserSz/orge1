...
Version: 1.0.1
Description: ESP32 bridge modülü için unit testler - Hex kod ve ACK davranışı doğrulama
"""
...
class TestESP32BridgeHexCodes:
    ...
    def setup_method(self):
        ...
        with patch("serial.Serial", return_value=self.mock_serial):
            ...
            # Hex kod testlerinde ACK bekleme davranışını sabitle:
            # _wait_for_ack her çağrıldığında başarılı bir ACK döndürecek.
            self.bridge._wait_for_ack = MagicMock(
                return_value={"CMD": "AUTH", "STATUS": "OK"}
            )
    ...
    def test_current_set_all_valid_values(self):
        ...
        for amperage in range(6, 33):
            ...
            assert (
                call_args[3] == amperage
            ), f"Amperage değeri yanlış: {call_args[3]} != {amperage}"
