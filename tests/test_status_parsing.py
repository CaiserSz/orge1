...
class TestStatusParsing:
    ...
    def test_status_with_newlines(self):
        ...
        # Newline içeren mesajlar parse edilemeyebilir (normal davranış)
        message = "<STAT;CP=0;\nSTATE=1;>"
        result = self.bridge._parse_status_message(message)

        # Newline içeren mesajlar None dönebilir veya parse edilebilir
        # Bu edge case için esnek kontrol yapalım
        if result is not None:
            assert result.get("CP") == 0 or result.get("STATE") == 1
        else:
            # Newline içeren mesajlar parse edilemeyebilir - bu normal
            pass
